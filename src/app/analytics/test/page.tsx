...
export default function AnalyticsTest() {
  ...
  const testAPI = async (endpoint: string, name: string) => {
    try {
      ...
      if (endpoint.includes('/export')) {
        ...
        console.log(`${name} result:`, { status: response.status, size: blob.size });
      } else {
  ...
  const runAllTests = async () => {
    ...
    await testAPI('/api/analytics/export?type=summary&dateRange=this-month', 'Export API');
  };

  const testExport = async (type: string) => {
    try {
      const response = await fetch(`/api/analytics/export?type=${type}&dateRange=this-month`);
      if (response.ok) {
        const blob = await response.blob();
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `test-${type}-${new Date().toISOString().split('T')[0]}.xlsx`;
  ...
  return (
    <div className="pt-20 p-6 min-h-screen">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-3xl font-bold mb-6">Analytics Functionality Test</h1>
        
        <div className="mb-6 space-y-4">
          <Button onClick={runAllTests} className="gradient-button text-white border-0">
            Run All Tests
          </Button>
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Button onClick={() => testExport('chart-data')} variant="outline">
                Test Chart Data Export
              </Button>
              <Button onClick={() => testExport('full-details')} variant="outline">
        ...
        {Object.keys(testResults).length === 0 && (
          <Card>
            <CardContent className="text-center py-8">
              <p className="text-gray-500">Click "Run All Tests" to test the analytics functionality</p>
            </CardContent>
          </Card>
        )}
      </div>
