  ...
  try {
    ...
    switch (exportType) {
      ...
      case 'full-details':
        ...
        XLSX.utils.book_append_sheet(workbook, fullDetailsWorksheet, 'Invoice Details');
        break;
      ...
      case 'summary':
        ...
        const summaryData = [
          ...
          { 'Metric': 'Pending Invoices', 'Value': invoices.filter(i => i.status === 'sent').length },
          { 'Metric': 'Draft Invoices', 'Value': invoices.filter(i => i.status === 'draft').length },
          { 'Metric': 'Overdue Invoices', 'Value': invoices.filter(i => i.status === 'overdue').length },
          { 'Metric': 'Unique Customers', 'Value': new Set(invoices.map(i => i.customerId)).size },
    ...
    return new NextResponse(excelBuffer, {
      ...
      headers: {
        ...
        'Content-Disposition': `attachment; filename="analytics-${exportType}-${new Date().toISOString().split('T')[0]}.xlsx"`
      }
    });
